...
public class TypedQueryPaginationService {
    ...
    /**
     * count typed query, but it is not like pagination query which we want Like
     * {@code SELECT COUNT(*) FROM ( MY_QUERY )}
     *
        ...
        sqmTranslator.getJdbcParamsBySqmParam()
            ...
            .forEach(entry -> entry.getValue()
                .forEach(values -> values
                    .stream()
                    .filter(
                        value -> indexByparameterBinders.containsKey(value.getParameterBinder()))
                    .findFirst()
                    .ifPresent(value -> {
